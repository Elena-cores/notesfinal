...
function passwordConfirmation() {
    ...
    //const hasNumber = /\d/;        // cualquier dígito [0-9]
    //const hasUpper = /[A-Z]/;      // cualquier mayúscula
    ...
    /**if (!hasNumber.test(password)) {
        document.getElementById('Error').innerHTML = 
          '<div class="alert alert-danger" role="alert">Password must contain at least one number!</div>';
        return false;
    }

    if (!hasUpper.test(password)) {
        document.getElementById('Error').innerHTML = 
          '<div class="alert alert-danger" role="alert">Password must contain at least one uppercase letter!</div>';
        return false;
    }
    */
    } else {
